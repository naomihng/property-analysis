...
#Please install.packakes("ggmap", type="source")
library(data.table)
library(ggplot2)
library(Hmisc)
library(hexbin)
...
ggmap(sing, extent = "device") + geom_density2d(data = dat, 
  aes(x = MapLon, y = MapLat), size = 0.3) + stat_density2d(data = dat, 
  aes(x = MapLon, y = MapLat, fill = ..level.., alpha = ..level..), size = 0.01, 
...
datmap <-dat[complete.cases(dat[,13:14])]
sm <- sing.map + geom_point(data=datmap, aes(x=MapLon, y=MapLat, color=`Asking (PSF)`)) + scale_color_gradient(low = "yellow",high ="red", limits=c(0,10000) ) +labs(y = "Latitude", x="Longitude")
sm
```
...
ggplot(dat, aes_string(x="`Property Type`")) + geom_bar() + coord_flip()+ ggtitle('Number of listing for each property type')
```
...
```{r}
ggplot(dat, aes(x=District, fill=property.type.new)) + geom_bar(position="fill") + coord_flip()+ ggtitle("Number of listing against district by Property type")
```
...
```{r}
ggplot(dat[!is.na(property.type.new)& property.type.new != "Others"], aes(x=`Asking (PSF)`)) + geom_density(aes(fill=property.type.new), alpha=0.5) + xlim(0,3000)
```
...
```{r boxplot}
ggplot(dat, aes(x=`property.type.new`, y=`Asking (PSF)`)) + geom_boxplot() + coord_flip()
```
...
```{r}
ggplot(dat[!is.na(property.type.new)& property.type.new != "Others"], aes(x=`property.type.new`, y=`Asking (PSF)`)) + geom_boxplot() + coord_flip()
```
...
```{r}
ggplot(dat[`Property Type`== "Condominium"], aes(x=reorder(`District`,`Asking (PSF)`, median),`Asking (PSF)`)) + geom_boxplot(outlier.color = "red") + theme(axis.text.x = element_text(angle = 90)) + labs(x= "District", y = "Asking PSF $ (Condominium)")
```
