...
```{r}
# get a mapping from District number to General Location for easier reading
district_url <- "https://www.ura.gov.sg/realEstateIIWeb/resources/misc/list_of_postal_districts.htm"
district.name <- read_html(district_url) %>% html_table(header=TRUE, trim=TRUE)
district.name <- data.table(district.name[[1]])
names(district.name) <- c("District", "PostalSector", "GeneralLocation")
district.name[, PostalSector := NULL][,GeneralLocation := gsub("\r\n\\s+"," ",GeneralLocation)]
dat <- merge(dat, district.name, by = "District", all=TRUE)
```
