...
dat[!is.na(District), District.new := District]
```
...
table(dat[is.na(MapLat) & !is.na(District), District])
```

Get Lat and Lon from District Name
```{r get lat lon from district}
...
dat <- merge(dat,district.data, by.x= "District", by.y="District.new", all.x = TRUE)
