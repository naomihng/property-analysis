...
x <- geocode(address)
## change far north west to Kranji
x[25,]<- geocode("Kranji ,Singapore", output = "latlon")
district.data<- cbind(district.data,x)
dat <- merge(dat,district.data, by= "District",all = TRUE)
```
Filling up empty latlon information
```{r merge lon and lat}
dat$MapLat<-ifelse(is.na(dat$MapLat),dat$lat,dat$MapLat)
dat$MapLon<-ifelse(is.na(dat$MapLon),dat$lon,dat$MapLon)
```

```{r remove outliers}
dat <- dat[!is.na(`QuikPro No`),]
dat$`QuikPro No`<-as.factor(dat$`QuikPro No`)
summary(dat$`QuikPro No`)
```
All Quickpro No are unique.  
